...
def create_periphs_tmp(addr_w, peripherals_list, out_file):
    ...
    for instance in peripherals_list:
        template_contents.extend(
            f"#define {instance.name}_BASE (({instance.name}<<({addr_w}-4-N_SLAVES_W))|(0xf<<({addr_w}-4)))\n"
        )

    # PLIC and CLINT base addresses
    template_contents.extend(
        f"#define CLINT0_BASE ((N_SLAVES-2<<({addr_w}-4-N_SLAVES_W))|(0xf<<({addr_w}-4)))\n"
    )
    template_contents.extend(
        f"#define PLIC0_BASE ((N_SLAVES-1<<({addr_w}-4-N_SLAVES_W))|(0xf<<({addr_w}-4)))\n"
    )
